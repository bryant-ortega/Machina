...
import { z } from 'zod'
import { registerVendor, type RegisterVendorResult } from './actions'

/**
 * Vendor self-registration form. Mirrors registration-form.tsx in
 * /register/dj so the look and error-handling stay identical across
 * the two registration paths.
 *
 * On success the server action redirects to /vendor/upload-w9.
 */
...
export function RegistrationForm() {
  ...
  function onSubmit(values: FormValues) {
    ...
    startTransition(async () => {
      const result = (await registerVendor(fd)) as
    ...
    return (
      <div className="space-y-3 rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-900 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-200">
        <p>
          ...
          can&apos;t be used to register as a vendor. Please use a different
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      <Field
        label="Company name"
        hint="The business name we'll address invoices and POs to."
      ...
      <Field
        label="Contact name"
        hint="The person we should reach out to."
      ...
      <Field label="Contact phone (optional)" error={errors.phone?.message}>
        <input
          type="tel"
          inputMode="tel"
          autoComplete="tel"
      ...
      <Field
        label="Location"
        hint="Where do you service? Pick the territory closest to your base."
