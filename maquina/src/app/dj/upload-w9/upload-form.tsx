...
import { uploadW9, type UploadW9Result } from './actions'

const MAX_BYTES = 10 * 1024 * 1024

/**
 * W-9 PDF upload form. Pre-validates file type + size on the client for
 * fast feedback; the server action revalidates before touching storage.
 */
export function UploadForm({
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result: UploadW9Result = await uploadW9(fd)
      if (result.ok) {
        setView({ kind: 'sent' })
...
function friendlyError(result: Exclude<UploadW9Result, { ok: true }>): string {
  switch (result.reason) {
    case 'unauth':
      return 'Your session expired. Sign in again.'
    case 'wrong_role':
      return 'This page is for DJ accounts only.'
    case 'no_file':
