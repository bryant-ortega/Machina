...
/**
 * /viewer/year — Phase 17g.
 *
 * The only page a `viewer`-role account can see. Mirrors the admin
 * year view (/views/year) but stripped of admin affordances:
 *   - no per-row link to /events/[id]/edit (viewers can't see that)
 *   - no export PDF
 *   - admin sidebar is replaced by the viewer-layout's slim header
 *
 * Data access:
 *   - `events` is gated by RLS — migration 0016's
 *     `events_select_viewer` policy lets viewers SELECT but not write.
 *   - `venues` was already readable by any signed-in user.
 *
 * URL state:
 *   ?year=YYYY (default = current year)
 *   ?confirmed_only=1 (default = off)
 *
 * Keep this page in sync with /(admin)/views/year/page.tsx when the
 * year view changes — they're intentionally a near-copy until we
 * factor out a shared loader.
 */
export default async function ViewerYearPage({
  searchParams,
}: {
  ...
  // Group events by month index (0..11). Months with no events stay out
